...
import { ResidancyService } from './residance.service'
import { IResidance } from '@/interface/residance'
export class AuthService {
  public user = new ResidancyService()

  public async Signup(user: IResidance) {
    try {
      const findUser = await models.Residance.findOne({ email: user.email })
      if (findUser) {
  ...
  public async resetPasswordLink(email: string) {
    try {
      const user = await models.Residance.findOne({ email: email })
      if (!user) {
        throw { success: false, message: 'User not found', code: 404 }
      }
  ...
  public async resetPassword(token: string, newPassword: string, confirmPassword: string) {
    try {
      ...
      const user = await models.Residance.findOne({ email: decoded.email })
